...
void floatTetWild::filter_outside_floodfill( Mesh& mesh, bool invert_faces )
{
	...
	for( int i = 0; i < tet_vertices.size(); i++ )
	{
		if( tet_vertices[ i ].isRemoved() )
			continue;
		if( tet_vertices[ i ].connTets.empty() )
...
void floatTetWild::manifold_edges( Mesh& mesh )
{
	...
	while( !edge_queue.empty() )
	{
		...
		for( int i = 0; i < tet_groups.size(); i++ )
		{
			...
			tet_vertices[ dup_v_id ].connTets.clear();
...
void floatTetWild::manifold_vertices( Mesh& mesh )
{
	...
	{
		std::map<int, bool> is_visited;
		for( int t_id : tet_vertices[ b_v_id ].connTets )
		{
...
void floatTetWild::manifold_surface( Mesh& mesh, VertexBuffer& V, SurfaceIndexBuffer& F )
{
	...
	for( auto& v : tet_vertices )
	{
		...
		for( int i = 0; i < v.connTets.size(); i++ )
		{
