...
void floatTetWild::output_info( Mesh& mesh, const AABBWrapper& tree )
{
	...
	for( int i = 0; i < tets.size(); i++ )
	{
		...
		{
			cout << "tet " << i << " inverted: " << tets[ i ][ 0 ] << " " << tets[ i ][ 1 ] << " " << tets[ i ][ 2 ] << " " << tets[ i ][ 3 ] << endl;
			// pausee();
		}
	}
			...
			if( tets[ i ].is_surface_fs[ j ] != NOT_SURFACE )
			{
				...
				for( int k = 0; k < 3; k++ )
				{
					if( !tet_vertices[ tets[ i ][ ( j + 1 + k ) % 4 ] ].is_on_surface )
					{
						...
						cout << "t " << i << ": " << tets[ i ][ 0 ] << " " << tets[ i ][ 1 ] << " " << tets[ i ][ 2 ] << " " << tets[ i ][ 3 ] << endl;
						cout << tets[ i ].is_surface_fs[ 0 ] << " " << tets[ i ].is_surface_fs[ 1 ] << " " << tets[ i ].is_surface_fs[ 2 ] << " "
							 << tets[ i ].is_surface_fs[ 3 ] << endl;
						cout << tet_vertices[ tets[ i ][ 0 ] ].is_on_surface << " " << tet_vertices[ tets[ i ][ 1 ] ].is_on_surface << " "
							 << tet_vertices[ tets[ i ][ 2 ] ].is_on_surface << " " << tet_vertices[ tets[ i ][ 3 ] ].is_on_surface << endl;
						// pausee();
					}
				}
			}
			...
			if( tets[ i ].is_bbox_fs[ j ] != NOT_BBOX )
			{
				for( int k = 0; k < 3; k++ )
				{
					if( !tet_vertices[ tets[ i ][ ( j + 1 + k ) % 4 ] ].is_on_bbox )
					{
						...
						cout << "t " << i << ": " << tets[ i ][ 0 ] << " " << tets[ i ][ 1 ] << " " << tets[ i ][ 2 ] << " " << tets[ i ][ 3 ] << endl;
						cout << (int)tets[ i ].is_bbox_fs[ 0 ] << " " << (int)tets[ i ].is_bbox_fs[ 1 ] << " " << (int)tets[ i ].is_bbox_fs[ 2 ] << " "
							 << (int)tets[ i ].is_bbox_fs[ 3 ] << endl;
						cout << tet_vertices[ tets[ i ][ 0 ] ].is_on_bbox << " " << tet_vertices[ tets[ i ][ 1 ] ].is_on_bbox << " "
							 << tet_vertices[ tets[ i ][ 2 ] ].is_on_bbox << " " << tet_vertices[ tets[ i ][ 3 ] ].is_on_bbox << endl;
						pausee();
					}
				}
	...
	for( int i = 0; i < tet_vertices.size(); i++ )
	{
		...
		if( tet_vertices[ i ].is_on_bbox )
		{
			...
			if( !is_found )
			{
				...
				for( int t_id : tet_vertices[ i ].conn_tets )
				{
					cout << "t " << t_id << ": " << tets[ t_id ][ 0 ] << " " << tets[ t_id ][ 1 ] << " " << tets[ t_id ][ 2 ] << " " << tets[ t_id ][ 3 ]
						 << endl;
					cout << tets[ t_id ].is_bbox_fs[ 0 ] << " " << tets[ t_id ].is_bbox_fs[ 1 ] << " " << tets[ t_id ].is_bbox_fs[ 2 ] << " "
						 << tets[ t_id ].is_bbox_fs[ 3 ] << endl;
					cout << tet_vertices[ tets[ t_id ][ 0 ] ].is_on_bbox << " " << tet_vertices[ tets[ t_id ][ 1 ] ].is_on_bbox << " "
						 << tet_vertices[ tets[ t_id ][ 2 ] ].is_on_bbox << " " << tet_vertices[ tets[ t_id ][ 3 ] ].is_on_bbox << endl;
				}
				// pausee();
			}
		}
		if( tet_vertices[ i ].is_on_surface )
		{
			...
			if( !is_found )
			{
				...
				for( int t_id : tet_vertices[ i ].conn_tets )
				{
					cout << "t " << t_id << ": " << tets[ t_id ][ 0 ] << " " << tets[ t_id ][ 1 ] << " " << tets[ t_id ][ 2 ] << " " << tets[ t_id ][ 3 ]
						 << endl;
					cout << tets[ t_id ].is_surface_fs[ 0 ] << " " << tets[ t_id ].is_surface_fs[ 1 ] << " " << tets[ t_id ].is_surface_fs[ 2 ] << " "
						 << tets[ t_id ].is_surface_fs[ 3 ] << endl;
					cout << tet_vertices[ tets[ t_id ][ 0 ] ].is_on_surface << " " << tet_vertices[ tets[ t_id ][ 1 ] ].is_on_surface << " "
						 << tet_vertices[ tets[ t_id ][ 2 ] ].is_on_surface << " " << tet_vertices[ tets[ t_id ][ 3 ] ].is_on_surface << endl;
				}
				// pausee();
			}
		}
		...
		for( int j = 0; j < 4; j++ )
		{
			if( SF_CONDITION )
			{
				...
				if( Predicates::orient_3d( tet_vertices[ t[ ( j + 1 ) % 4 ] ].pos, tet_vertices[ t[ ( j + 2 ) % 4 ] ].pos,
					  tet_vertices[ t[ ( j + 3 ) % 4 ] ].pos, tet_vertices[ t[ j ] ].pos ) == Predicates::ORI_POSITIVE )
...
void floatTetWild::apply_sizingfield( Mesh& mesh, AABBWrapper& tree )
{
	...
	for( int i = 0; i < 20; i++ )
	{
		...
		double max_energy = mesh.get_max_energy();
		cout << "/////////" << i << " " << max_energy << endl;
		if( ( tmp_num_tets - num_tets ) / num_tets < 0.02 && max_energy < mesh.params.stop_energy )	 // refinement and quality enough
			break;
		num_tets = tmp_num_tets;
	}
}
