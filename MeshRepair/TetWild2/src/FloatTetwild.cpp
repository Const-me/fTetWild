...
{
	int tetrahedralization( GEO::Mesh& sf_mesh, Parameters params, Eigen::MatrixXd& V, Eigen::MatrixXi& T, int boolean_op, bool skip_simplify )
	{
		if( !sf_mesh.facets.are_simplices() )
			GEO::mesh_repair( sf_mesh, GEO::MeshRepairMode( GEO::MESH_REPAIR_TRIANGULATE | GEO::MESH_REPAIR_QUIET ) );
		...
		{
			input_vertices[ i ] << ( sf_mesh.vertices.point( i ) )[ 0 ], ( sf_mesh.vertices.point( i ) )[ 1 ], ( sf_mesh.vertices.point( i ) )[ 2 ];
		}
		...
		logger().info( "" );
		stats().record( StateInfo::cutting_id, timer.getElapsedTimeInSec(), mesh.get_v_num(), mesh.get_t_num(), mesh.get_max_energy(), mesh.get_avg_energy(),
		...
		logger().info( "" );
		stats().record(
		  StateInfo::optimization_id, timer.getElapsedTimeInSec(), mesh.get_v_num(), mesh.get_t_num(), mesh.get_max_energy(), mesh.get_avg_energy() );
		...
		stats().record( StateInfo::wn_id, timer.getElapsedTimeInSec(), mesh.get_v_num(), mesh.get_t_num(), mesh.get_max_energy(), mesh.get_avg_energy() );
