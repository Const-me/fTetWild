...
namespace floatTetWild
{
	class AABBWrapper
	{
	  public:
		...
		inline bool is_out_sf_envelope( const std::vector<GEO::vec3>& ps, const Scalar eps_2, GEO::index_t prev_facet = GEO::NO_FACET ) const
		{
		...
		inline bool is_out_b_envelope( const std::vector<GEO::vec3>& ps, const Scalar eps_2, GEO::index_t prev_facet = GEO::NO_FACET ) const
		{
			GEO::vec3 nearest_point;
			double sq_dist = std::numeric_limits<double>::max();
		...
		inline bool is_out_tmp_b_envelope( const std::vector<GEO::vec3>& ps, const Scalar eps_2, GEO::index_t prev_facet = GEO::NO_FACET ) const
		{
			GEO::vec3 nearest_point;
			double sq_dist = std::numeric_limits<double>::max();
		...
		inline bool is_out_sf_envelope( const Vector3& p, const Scalar eps_2, GEO::index_t& prev_facet, double& sq_dist, GEO::vec3& nearest_p ) const
		{
			GEO::vec3 geo_p( p[ 0 ], p[ 1 ], p[ 2 ] );
			return is_out_sf_envelope( geo_p, eps_2, prev_facet, sq_dist, nearest_p );
		}
		inline bool is_out_sf_envelope( const GEO::vec3& geo_p, const Scalar eps_2, GEO::index_t& prev_facet, double& sq_dist, GEO::vec3& nearest_p ) const
		{
			if( prev_facet != GEO::NO_FACET )
			{
		...
		// fortest
		inline Scalar dist_sf_envelope( const std::vector<GEO::vec3>& ps, const Scalar eps_2, GEO::index_t prev_facet = GEO::NO_FACET ) const
