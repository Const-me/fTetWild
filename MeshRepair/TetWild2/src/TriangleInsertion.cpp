...
void floatTetWild::optimize_non_surface( const std::vector<Vector3>& input_vertices, const std::vector<Vector3i>& input_faces,
  const std::vector<int>& input_tags, std::vector<bool>& is_face_inserted, const std::vector<std::array<std::vector<int>, 4>>& track_surface_fs, Mesh& mesh,
...
bool floatTetWild::insert_multi_triangles( int insert_f_id, const std::vector<Vector3>& input_vertices, const std::vector<Vector3i>& input_faces,
  const std::vector<int>& input_tags, const std::vector<std::vector<int>>& conn_fs, const std::vector<Vector3>& ns, std::vector<bool>& is_visited,
  std::vector<int>& f_ids, Mesh& mesh, std::vector<std::array<std::vector<int>, 4>>& track_surface_fs, AABBWrapper& tree, bool is_again )
{
	...
	for( int f_id : f_ids )
	{
		std::array<Vector3, 3> tmp_vs = {
		  { input_vertices[ input_faces[ f_id ][ 0 ] ], input_vertices[ input_faces[ f_id ][ 1 ] ], input_vertices[ input_faces[ f_id ][ 2 ] ] } };
  ...
  std::vector<std::array<int, 3>>& known_surface_fs, std::vector<std::array<int, 3>>& known_not_surface_fs )
{
	...
	for( int I = 0; I < b_edge_infos.size(); I++ )
	{
			  ...
			  new_track_surface_fs, modified_t_ids ) )
		{
			...
			{
#ifdef NEW_ENVELOPE
				if( tree.is_out_sf_envelope_exact( { { mesh.tet_vertices[ f[ 0 ] ].pos, mesh.tet_vertices[ f[ 1 ] ].pos, mesh.tet_vertices[ f[ 2 ] ].pos } } ) )
				{
				...
				sample_triangle(
				  { { mesh.tet_vertices[ f[ 0 ] ].pos, mesh.tet_vertices[ f[ 1 ] ].pos, mesh.tet_vertices[ f[ 2 ] ].pos } }, ps, mesh.params.dd );
	...
	for( int t_id = 0; t_id < track_surface_fs.size(); t_id++ )
	{
		for( int j = 0; j < 4; j++ )
		{
			...
			else
			{
				...
				//                std::array<GEO2::vec3, 3> t_vs_geo = {{GEO2::vec3(tp1_3d[0], tp1_3d[1], tp1_3d[2]),
				//                                                             GEO2::vec3(tp2_3d[0], tp2_3d[1], tp2_3d[2]),
				//                                                             GEO2::vec3(tp3_3d[0], tp3_3d[1], tp3_3d[2])}};
				//                for (int f_id: f_ids) {
				//                    std::array<GEO2::vec3, 3> f_vs_geo;
				//                    for (int k = 0; k < 3; k++) {
				//                        f_vs_geo[k] = GEO2::vec3(input_vertices[input_faces[f_id][k]][0],
				//                                                input_vertices[input_faces[f_id][k]][1],
				//                                                input_vertices[input_faces[f_id][k]][2]);
				//                    }
				//                    for (int k = 0; k < 3; k++) {
				//                        double dist_2 = GEO2::Geom::point_triangle_squared_distance(t_vs_geo[k], f_vs_geo[0],
...
void floatTetWild::check_track_surface_fs( Mesh& mesh, std::vector<std::array<std::vector<int>, 4>>& track_surface_fs,
  const std::vector<Vector3>& input_vertices, const std::vector<Vector3i>& input_faces, const std::vector<int>& sorted_f_ids )
{
	return;
	...
	for( int i = 0; i < sorted_f_ids.size(); i++ )
	{
		...
		if( covered_fs_infos[ f_id ].empty() )	// not inserted
			continue;
		std::array<Vector3, 3> f_vs = {
		  { input_vertices[ input_faces[ f_id ][ 0 ] ], input_vertices[ input_faces[ f_id ][ 1 ] ], input_vertices[ input_faces[ f_id ][ 2 ] ] } };
		int t = get_t( f_vs[ 0 ], f_vs[ 1 ], f_vs[ 2 ] );
		std::array<Vector2, 3> f_vs_2d = { { to_2d( input_vertices[ input_faces[ f_id ][ 0 ] ], t ), to_2d( input_vertices[ input_faces[ f_id ][ 1 ] ], t ),
		  to_2d( input_vertices[ input_faces[ f_id ][ 2 ] ], t ) } };
		//
		...
		for( int n = 0; n < covered_fs_infos[ f_id ].size(); n++ )
		{
			...
			std::array<Vector3, 3> t_vs = { { mesh.tet_vertices[ mesh.tets[ t_id ][ ( j + 1 ) % 4 ] ].pos,
			  mesh.tet_vertices[ mesh.tets[ t_id ][ ( j + 2 ) % 4 ] ].pos, mesh.tet_vertices[ mesh.tets[ t_id ][ ( j + 3 ) % 4 ] ].pos } };
			...
			//                    continue;
			//                double dist_2 = GEO2::Geom::point_triangle_squared_distance(
			//                        GEO2::vec3(t_vs[r][0], t_vs[r][1], t_vs[r][2]),
			//                        GEO2::vec3(f_vs[0][0], f_vs[0][1], f_vs[0][2]),
			//                        GEO2::vec3(f_vs[1][0], f_vs[1][1], f_vs[1][2]),
			//                        GEO2::vec3(f_vs[2][0], f_vs[2][1], f_vs[2][2]));
		...
		for( const auto& p : ps )
		{
			...
			for( const auto& info : covered_fs_infos[ f_id ] )
			{
				int t_id = info.first;
				int j = info.second;
				std::array<int, 3> f = { { mesh.tets[ t_id ][ ( j + 1 ) % 4 ], mesh.tets[ t_id ][ ( j + 2 ) % 4 ], mesh.tets[ t_id ][ ( j + 3 ) % 4 ] } };
				std::array<GEO2::vec3, 3> vs;
				for( int k = 0; k < 3; k++ )
				{
		...
		// output input/tet triangles in different colors
		{
			...
			igl::writeOFF( std::to_string( i ) + "_covered_tet_fs_" + std::to_string( f_id ) + "_" + std::to_string( is_inside ) + ".off", V, F, C );
		}
		{
			Eigen::MatrixXd V( 3, 3 ), C( 3, 3 );
			Eigen::MatrixXi F( 1, 3 );
			...
			F.row( 0 ) << 0, 1, 2;
			igl::writeOFF( std::to_string( i ) + "_covered_input_f_" + std::to_string( f_id ) + "_" + std::to_string( is_inside ) + ".off", V, F, C );
		}
		//        pausee();
	}
}
