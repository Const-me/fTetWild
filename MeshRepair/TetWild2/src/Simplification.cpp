...
void floatTetWild::collapsing( std::vector<Vector3>& input_vertices, std::vector<Vector3i>& input_faces, const AABBWrapper& tree, const Parameters& params,
  std::vector<bool>& v_is_removed, std::vector<bool>& f_is_removed, std::vector<std::unordered_set<int>>& conn_fs )
{
	...
	const auto build_edges = [ & ]()
	{
		...
		tbb::parallel_for( size_t( 0 ), input_faces.size(),
		  [ & ]( size_t f_id )
		  {
	...
	auto remove_an_edge = [ & ]( int v1_id, int v2_id, const std::vector<int>& n12_f_ids )
	{
		...
		{
			Vector3 old_nv = ( input_vertices[ input_faces[ f_id ][ 1 ] ] - input_vertices[ input_faces[ f_id ][ 2 ] ] )
			...
			for( int j = 0; j < 3; j++ )
			{
				...
				{
					Vector3 new_nv = ( input_vertices[ input_faces[ f_id ][ mod3( j + 1 ) ] ] - input_vertices[ input_faces[ f_id ][ mod3( j + 2 ) ] ] )
			...
			for( int j = 0; j < 3; j++ )
			{
				if( input_faces[ f_id ][ j ] == v1_id || input_faces[ f_id ][ j ] == v2_id )
				{
					const std::array<Vector3, 3> tri = {
					  { p, input_vertices[ input_faces[ f_id ][ mod3( j + 1 ) ] ], input_vertices[ input_faces[ f_id ][ mod3( j + 2 ) ] ] } };
	...
	do
	{
		...
		tbb::parallel_for( size_t( 0 ), safe_set.size(),
		  [ & ]( size_t i )
		  {
		...
		// cleanup conn_fs
		tbb::parallel_for( size_t( 0 ), conn_fs.size(),
		  [ & ]( size_t i )
		  {
  ...
  std::vector<bool>& v_is_removed, std::vector<bool>& f_is_removed, std::vector<std::unordered_set<int>>& conn_fs )
{
	...
	while( !sm_queue.empty() )
	{
		...
		for( int j = 0; j < 3; j++ )
		{
			if( n_v_ids[ 0 ] < 0 && input_faces[ n12_f_ids[ 0 ] ][ j ] != v1_id && input_faces[ n12_f_ids[ 0 ] ][ j ] != v2_id )
				n_v_ids[ 0 ] = input_faces[ n12_f_ids[ 0 ] ][ j ];

			if( n_v_ids[ 1 ] < 0 && input_faces[ n12_f_ids[ 1 ] ][ j ] != v1_id && input_faces[ n12_f_ids[ 1 ] ][ j ] != v2_id )
		...
		if( cos_a0 > -0.999 )
		{													   // maybe it's for avoiding numerical issue
		...
		// check quality
		Scalar cos_a0_new = get_angle_cos( input_vertices[ v1_id ], input_vertices[ n_v_ids[ 0 ] ], input_vertices[ n_v_ids[ 1 ] ] );
		Scalar cos_a1_new = get_angle_cos( input_vertices[ v2_id ], input_vertices[ n_v_ids[ 0 ] ], input_vertices[ n_v_ids[ 1 ] ] );
		...
		//        if(!is_valid)
		//            continue;
		if( is_out_envelope( { { input_vertices[ v1_id ], input_vertices[ n_v_ids[ 0 ] ], input_vertices[ n_v_ids[ 1 ] ] } }, tree, params ) ||
			is_out_envelope( { { input_vertices[ v2_id ], input_vertices[ n_v_ids[ 0 ] ], input_vertices[ n_v_ids[ 1 ] ] } }, tree, params ) )
		{
			continue;
		}
...
bool floatTetWild::is_out_envelope( const std::array<Vector3, 3>& vs, const AABBWrapper& tree, const Parameters& params )
{
	...
	return sampleTriangleAndCheckOut( vs, params.dd_simplification, params.eps_2_simplification, tree, prev_facet );
#endif
#endif
