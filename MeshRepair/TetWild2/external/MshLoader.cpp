...
MshLoader::MshLoader( const std::string& filename )
{
	...
	if( m_binary )
	{
		...
		if( one != 1 )
		{
			std::cerr << "Warning: binary msh file " << filename << " is saved with different endianness than this machine." << std::endl;
			throw NOT_IMPLEMENTED;
		}
	}
...
int MshLoader::num_nodes_per_elem_type( int elem_type )
{
	...
	case 5:
		...
		break;
	default:
		std::cerr << "Warning: Element type (" << elem_type << ") is not supported yet." << std::endl;
		throw NOT_IMPLEMENTED;
	}
	assert( nodes_per_element <= INT_MAX );
	return (int)nodes_per_element;
}