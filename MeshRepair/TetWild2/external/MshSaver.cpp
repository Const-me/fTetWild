...
namespace floatTetWild
{
	...
	void MshSaver::save_mesh( const VectorF& nodes, const VectorI& elements, const VectorI& components, size_t dim, MshSaver::ElementType type )
	{
		if( dim != 2 && dim != 3 )
		{
			std::stringstream err_msg;
	...
	void MshSaver::save_elements( const VectorI& elements, const VectorI& components, MshSaver::ElementType type )
	{
	...
	void MshSaver::save_vector_field( const std::string& fieldname, const VectorF& field )
	{
		...
		else
		{
			for( size_t i = 0; i < m_num_nodes; i++ )
			{
				...
				if( m_dim == 3 )
				{
					fout << node_idx << " " << field[ i * 3 ] << " " << field[ i * 3 + 1 ] << " " << field[ i * 3 + 2 ] << std::endl;
				}
				else if( m_dim == 2 )
				{
					fout << node_idx << " " << field[ i * 2 ] << " " << field[ i * 2 + 1 ] << " " << zero << std::endl;
				}
			}
		}
		fout << "$EndNodeData" << std::endl;
	...
	void MshSaver::save_elem_vector_field( const std::string& fieldname, const VectorF& field )
	{
		...
		else
		{
			for( size_t i = 0; i < m_num_elements; i++ )
			{
				...
				if( m_dim == 3 )
				{
					fout << elem_idx << " " << field[ i * 3 ] << " " << field[ i * 3 + 1 ] << " " << field[ i * 3 + 2 ] << std::endl;
				}
				else if( m_dim == 2 )
				{
					fout << elem_idx << " " << field[ i * 2 ] << " " << field[ i * 2 + 1 ] << " " << zero << std::endl;
				}
			}
		}
	...
	void MshSaver::save_elem_tensor_field( const std::string& fieldname, const VectorF& field )
	{
		...
		if( m_binary )
		{
			for( size_t i = 0; i < m_num_elements; i++ )
			{
				...
				if( m_dim == 3 )
				{
					...
					Float tensor[ 9 ] = { val[ 0 ], val[ 5 ], val[ 4 ], val[ 5 ], val[ 1 ], val[ 3 ], val[ 4 ], val[ 3 ], val[ 2 ] };
		...
		else
		{
			for( size_t i = 0; i < m_num_elements; i++ )
			{
				...
				{
					const VectorF& val = field.segment( i * 6, 6 );
					fout << elem_idx << " " << val[ 0 ] << " " << val[ 5 ] << " " << val[ 4 ] << " " << val[ 5 ] << " " << val[ 1 ] << " " << val[ 3 ] << " "
						 << val[ 4 ] << " " << val[ 3 ] << " " << val[ 2 ] << std::endl;
				}
				else if( m_dim == 2 )
				{
					const VectorF& val = field.segment( i * 3, 3 );
					fout << elem_idx << " " << val[ 0 ] << " " << val[ 2 ] << " " << zero << " " << val[ 2 ] << " " << val[ 1 ] << " " << zero << " " << zero
						 << " " << zero << " " << zero << std::endl;
				}
			}
		}
