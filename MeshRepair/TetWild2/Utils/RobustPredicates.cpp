...
namespace RodustPredicates
{
	...
	// Masking away the sign bit is not terribly hard to accomplish in C, at least when compiling for AMD64:
	static const uint64_t s_absMask = ( ~( (uint64_t)0 ) ) >> 1;

	static __forceinline double Absolute( double v )
	{
		__m128d vec = _mm_set_sd( v );
		const __m128d absMask = _mm_load_sd( (const double*)&s_absMask );
		vec = _mm_and_pd( vec, absMask );
...
#define PRINT_LITERAL( x ) dbgPrintCppLiteral( #x, x )
		PRINT_LITERAL( splitter );
		PRINT_LITERAL( epsilon );
		PRINT_LITERAL( resulterrbound );
		PRINT_LITERAL( ccwerrboundA );
		PRINT_LITERAL( ccwerrboundB );
		PRINT_LITERAL( ccwerrboundC );
		PRINT_LITERAL( o3derrboundA );
		PRINT_LITERAL( o3derrboundB );
		PRINT_LITERAL( o3derrboundC );
		PRINT_LITERAL( iccerrboundA );
		PRINT_LITERAL( iccerrboundB );
		PRINT_LITERAL( iccerrboundC );
		PRINT_LITERAL( isperrboundA );
		PRINT_LITERAL( isperrboundB );
		PRINT_LITERAL( isperrboundC );
#undef PRINT_LITERAL
	}
#endif
	/*****************************************************************************/
	...
	static int fast_expansion_sum_zeroelim( int elen, const double* e, int flen, const double* f, double* h ) /* h cannot be e or f. */
	{
		double Q;
		double Qnew;
		double hh;
	...
	static int scale_expansion_zeroelim( int elen, const double* e, double b, double* h ) /* e and h cannot be the same. */
	{
		double Q, sum;
		double hh;
		double product1;
	...
	double insphereadapt( const double* pa, const double* pb, const double* pc, const double* pd, const double* pe, double permanent )
	{
		...
		det +=
		  ( ( ( bex * bex + bey * bey + bez * bez ) * ( ( cez * daeps + dez * aceps + aez * cdeps ) + ( ceztail * da3 + deztail * ac3 + aeztail * cd3 ) ) +
